...
DRUM_NOTES = {"kick": 36, "snare": 38, "clap": 39, "snap": 40, "hat_closed": 42, "tick": 44,
              "tom_low": 45, "hat_open": 46, "tom_mid": 47, "crash": 49, "ride": 51,
              "crash_roll": 57, "shaker": 70}
...
class DrumKit:
    ...
    def set_patch(self, patch: DrumPatchModel) -> None:
        ...
        snap = GatedReverb(sr, 120, size=0.7, hold=0.12, threshold=0.2,
                           mix=sn.reverb_mix).process(_stereo(burst * env + thump))[:, 0]
        ...
        ping = sum(np.sin(2 * np.pi * f * t) * np.exp(-t / (rd.decay * 0.6)) / (i + 1)
        ...
        t = _t(sr, sh.decay * 4)
        shaker = (_filt("bp", rng.uniform(-1, 1, len(t)), sh.cutoff, 0.35, sr)
                  * np.minimum(1.0, t / 0.004) * np.exp(-t / sh.decay))
        ...
        self.samples = {
            ...
            "snap": _stereo(snap) * sn.gain, "ride": _stereo(ride) * rd.gain,
            "shaker": _stereo(shaker) * sh.gain, "tick": _stereo(tick) * tk.gain,
        }
